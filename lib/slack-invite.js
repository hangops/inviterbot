...
function invite({ org, token, email, channel, logger, inviteMode, inviteUrl }, fn) {
  ...
  if (mode === 'api') {
    ...
    return inviteViaAPI({ org, token, email, channel, logger }, fn);
  }

  // Default mode: Validate and redirect to shared invite link
  return inviteViaLink({ org, email, logger, inviteUrl }, fn);
}
...
function inviteViaAPI({ org, token, email, channel, logger }, fn) {
  ...
  client.admin.users.invite(options)
    .then((response) => {
      if (!response.ok) {
        if (logger) logger(`Error sending an invite to ${email}: ${response.error}`);
    ...
    .catch((error) => {
      if (logger) logger(`Error sending an invite to ${email}: ${error.message}`);
      return fn(error);
    });
}
